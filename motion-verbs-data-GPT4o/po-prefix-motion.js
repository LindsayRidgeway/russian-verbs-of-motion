// По- prefix motion verbs data
// Motion verbs with по- prefix (setting off / brief motion)

window.пoMotionData = {
  prefix: "по",
  groups: [
    {
      tense: "past",
      aspect: "perfective",
      travelMode: "by-foot",
      sentences: [
        { english: "I set off toward the forest (male)", russian: "я пошёл в лес" },
        { english: "I set off toward the forest (female)", russian: "я пошла в лес" },
        { english: "You headed to the mountain trail (male)", russian: "ты пошёл к горной тропе" },
        { english: "You headed to the mountain trail (female)", russian: "ты пошла к горной тропе" },
        { english: "He started walking to the lake", russian: "он пошёл к озеру" },
        { english: "She set off to the meadow", russian: "она пошла на луг" },
        { english: "We headed to the campsite", russian: "мы пошли на место стоянки" },
        { english: "You all set off to the creek", russian: "вы пошли к ручью" },
        { english: "They started walking to the valley", russian: "они пошли в долину" }
      ]
    },
    {
      tense: "past",
      aspect: "perfective",
      travelMode: "by-vehicle",
      sentences: [
        { english: "I drove off to the theater (male)", russian: "я поехал в театр" },
        { english: "I drove off to the theater (female)", russian: "я поехала в театр" },
        { english: "You drove off to the museum (male)", russian: "ты поехал в музей" },
        { english: "You drove off to the museum (female)", russian: "ты поехала в музей" },
        { english: "He drove off to the cinema", russian: "он поехал в кинотеатр" },
        { english: "She drove off to the mall", russian: "она поехала в торговый центр" },
        { english: "We drove off to the airport", russian: "мы поехали в аэропорт" },
        { english: "You all drove off to the stadium", russian: "вы поехали на стадион" },
        { english: "They drove off to the restaurant", russian: "они поехали в ресторан" }
      ]
    },
    {
      tense: "past",
      aspect: "imperfective",
      direction: "multidirectional",
      travelMode: "by-foot",
      sentences: [
        { english: "I walked around the lake area for a while (male)", russian: "я походил около озера" },
        { english: "I walked around the lake area for a while (female)", russian: "я походила около озера" },
        { english: "You walked in the woods for a bit (male)", russian: "ты походил по лесу" },
        { english: "You walked in the woods for a bit (female)", russian: "ты походила по лесу" },
        { english: "He walked by the stream for a while", russian: "он походил у ручья" },
        { english: "She walked up the hills for a bit", russian: "она походила по холмам" },
        { english: "We walked in the canyon for a while", russian: "мы походили в ущелье" },
        { english: "You all walked around the meadow for a bit", russian: "вы походили по лугу" },
        { english: "They walked in the valley for a while", russian: "они походили по долине" }
      ]
    },
    {
      tense: "past",
      aspect: "imperfective",
      direction: "multidirectional",
      travelMode: "by-vehicle",
      sentences: [
        { english: "I drove around to concerts for a while (male)", russian: "я поездил на концерты" },
        { english: "I drove around to concerts for a while (female)", russian: "я поездила на концерты" },
        { english: "You drove to the gym for a bit (male)", russian: "ты поездил в спортзал" },
        { english: "You drove to the gym for a bit (female)", russian: "ты поездила в спортзал" },
        { english: "He drove around town for a while", russian: "он поездил по городу" },
        { english: "She drove to work for a while", russian: "она поездила на работу" },
        { english: "We drove to different places for a bit", russian: "мы поездили в разные места" },
        { english: "You all drove to various events for a while", russian: "вы поездили на разные мероприятия" },
        { english: "They drove to shopping centers for a bit", russian: "они поездили по торговым центрам" }
      ]
    },
    {
      tense: "future",
      aspect: "perfective",
      travelMode: "by-foot",
      sentences: [
        { english: "I will head to the lake (male)", russian: "я пойду к озеру" },
        { english: "I will head to the lake (female)", russian: "я пойду к озеру" },
        { english: "You will set off to the forest (male)", russian: "ты пойдёшь в лес" },
        { english: "You will set off to the forest (female)", russian: "ты пойдёшь в лес" },
        { english: "He will head up the hill", russian: "он пойдёт на холм" },
        { english: "She will set off to the playground", russian: "она пойдёт на площадку" },
        { english: "We will set off down the trail", russian: "мы пойдём по тропе" },
        { english: "You all will head to the park", russian: "вы пойдёте в парк" },
        { english: "They will head to the valley", russian: "они пойдут в долину" }
      ]
    },
    {
      tense: "future",
      aspect: "perfective",
      travelMode: "by-vehicle",
      sentences: [
        { english: "I will drive off to the cinema (male)", russian: "я поеду в кинотеатр" },
        { english: "I will drive off to the cinema (female)", russian: "я поеду в кинотеатр" },
        { english: "You will drive off to the airport (male)", russian: "ты поедешь в аэропорт" },
        { english: "You will drive off to the airport (female)", russian: "ты поедешь в аэропорт" },
        { english: "He will drive off to the hospital", russian: "он поедет в больницу" },
        { english: "She will drive off to the meeting", russian: "она поедет на встречу" },
        { english: "We will drive off to the park", russian: "мы поедем в парк" },
        { english: "You all will drive off to the stadium", russian: "вы поедете на стадион" },
        { english: "They will drive off to the restaurant", russian: "они поедут в ресторан" }
      ]
    }
  ]
};