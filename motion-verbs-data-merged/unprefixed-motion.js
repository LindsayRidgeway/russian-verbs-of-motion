// Unprefixed motion verbs — merged & corrected
// Families: идти/ходить (on foot), ехать/ездить (by vehicle)
// Rules encoded here:
// 1) Multidirectional (habitual/various directions) uses ходить / ездить.
// 2) Unidirectional (one course, en route now) uses идти / ехать.
// 3) Perfective "set off" exists only as ПО- prefixed forms: пойти / поехать.
//    Therefore: there is NO multidirectional + perfective group.
// 4) Present: идти/ехать denote motion in progress (often to/toward a destination);
//    ходить/ездить denote habitual/repeated. "в/на + Acc" = to; "к + Dat" = toward;
//    "по + Dat"/"вдоль + Gen" = along/around. We keep English in that spirit.
// 5) Past gender marking kept where forms differ (шел/шла; пошёл/пошла; поехал/поехала).
...
window.unprefixedMotionData = {
  ...
  groups: [
    ...
    // NOTE: Multidirectional + perfective is impossible → no groups for that combination.
